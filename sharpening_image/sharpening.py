...
grey_img, u, v = cv2.split(cv2.cvtColor(origin_img, cv2.COLOR_BGR2YUV))

# get sobel_img and laplacian_img
height = grey_img.shape[0]
width = grey_img.shape[1]

sobel_img = np.zeros( [ height+4, width+4 ])
laplacian_img = np.zeros( [ height+4, width+4 ])
padding_origin = np.zeros( [ height+4, width+4 ])
padding_origin[2:-2, 2:-2] = grey_img
...
for y in range(cov_range, height+4-cov_range):
    ...
    for x in range(cov_range, width+4-cov_range):
        ...
        value = abs(np.sum(part_img[:,:] * sobel_x)) + abs(np.sum(part_img[:,:] * sobel_y))
        sobel_img[y,x] = value

        # convolution with laplacian
        value = np.sum(part_img[:,:] * laplacian)
        laplacian_img[y,x] = value
...
laplacian_sharp = grey_img + laplacian_img
...
# get result_grey_img
result3_img = grey_img + masked3_laplacian_img
cv2.imwrite( 'images/output_img/sharpend3_grey_img.jpg', result3_img )

result5_img = grey_img + masked5_laplacian_img
cv2.imwrite( 'images/output_img/sharpend5_grey_img.jpg', result5_img )

result7_img = grey_img + masked9_laplacian_img
cv2.imwrite( 'images/output_img/sharpend7_grey_img.jpg', result7_img )

result9_img = grey_img + masked9_laplacian_img
cv2.imwrite( 'images/output_img/sharpend9_grey_img.jpg', result9_img )

result5_img = np.clip(result5_img,0,255)
result5_img = np.uint8(result5_img)
cv2.imwrite( 'images/output_img/sharpend5_greyyy_img.jpg', result5_img )
result_img = cv2.merge([result5_img, u, v])
result_img = cv2.cvtColor(result_img, cv2.COLOR_YUV2BGR)
cv2.imwrite( 'images/output_img/sharpend5_img.jpg', result_img )